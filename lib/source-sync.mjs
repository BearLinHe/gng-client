...
const orderQuery = `
  select
    ...
    inbound_schedule.planned_unload_at::text as unload_date,
    o.operation_mode,
    coalesce(
      nullif(
        case
...
const inventorySummaryQuery = `
  with lot_scope as (
    select
      ...
      il.order_detail_id,
      il.pallet_count,
      il.pallet_counts_verified,
      il.remaining_pallet_count,
      od.estimated_pallets
    from wms.inventory_lots il
    join public.orders o
      on o.order_id = il.order_id
    join public.order_detail od
      on od.id = il.order_detail_id
    join wms.inbound_receipt ir
      on ir.inbound_receipt_id = il.inbound_receipt_id
    left join public.customers c
      on c.id = o.customer_id
    where c.id is not null
      and il.storage_location_code is not null
      and ir.status = 'received'
      and o.order_number is not null
      and btrim(o.order_number) <> ''
      and coalesce(o.status, '') not in ('archived', 'cancelled', 'canceled')
      and coalesce(il.status, '') not in ('archived', 'cancelled', 'canceled')
  ),
  appointment_totals as (
    select
      ...
      coalesce(
        sum(
          greatest(
            coalesce(adl.estimated_pallets, 0)
              - coalesce(adl.rejected_pallets, 0),
            0
          )
        ) filter (
          where da.confirmed_start is not null
            and da.confirmed_start::date <= current_date
        ),
        0
      )::int as expired_effective_pallets
    from oms.appointment_detail_lines adl
    join oms.delivery_appointments da
      on da.appointment_id = adl.appointment_id
    where coalesce(da.enabled, true) = true
    group by adl.order_detail_id
  )
  select
    ls.source_customer_id,
    coalesce(
      sum(
        case
          when ls.pallet_counts_verified = true
            then coalesce(ls.remaining_pallet_count, 0)
          else
            coalesce(ls.pallet_count, ls.estimated_pallets, 0)
            - coalesce(at.expired_effective_pallets, 0)
        end
      ),
      0
    )::int as inventory_remaining_pallets
  from lot_scope ls
  left join appointment_totals at
    on at.order_detail_id = ls.order_detail_id
  group by ls.source_customer_id
`;
  ...
  try {
    ...
    for (const row of sourceRows) {
      ...
      customerMap.set(row.source_customer_id, {
        ...
        inventoryRemainingPallets: 0,
      });
    }

    logger("Reading source inventory summary...");
    const inventorySummaryResult = await sourceClient.query(inventorySummaryQuery);
    for (const row of inventorySummaryResult.rows) {
      const customer = customerMap.get(row.source_customer_id);
      if (!customer) continue;
      customer.inventoryRemainingPallets = Number(
        row.inventory_remaining_pallets ?? 0,
      );
    }
    ...
    logger("Detecting source warehouse changes...");
    await recordWarehouseDetailChangeEvents(
      systemClient,
      warehouseDetailPayload,
    );
    ...
    logger("Detecting source appointment changes...");
    await recordWarehouseAppointmentChangeEvents(
      systemClient,
      warehouseAppointmentPayload,
    );
...
async function ensureSystemSchema(client) {
  await client.query(`
    create table if not exists public.portal_customers (
      ...
      inventory_remaining_pallets integer not null default 0,
      show_appointment_number boolean not null default true,
      show_delivery_date boolean not null default true,
      show_effective_pallets boolean not null default true,
      show_pod boolean not null default true,
      show_bol boolean not null default true,
      show_source_change_notifications boolean not null default true,
      reveal_delivery_details_after_pickup boolean not null default false,
    ...
    create table if not exists public.portal_containers (
      ...
      manual_extra_charge_responsibility text,
      synced_at timestamptz not null default now(),
      updated_at timestamptz not null default now()
    );
    ...
    create table if not exists public.portal_warehouse_details (
      ...
      manual_customer_note text,
      manual_window_period text,
      manual_window_period_override boolean not null default false,
  ...
  await client.query(`
    alter table public.portal_customers
      ...
      add column if not exists inventory_remaining_pallets integer not null default 0,
      add column if not exists show_appointment_number boolean not null default true,
      add column if not exists show_delivery_date boolean not null default true,
      add column if not exists show_effective_pallets boolean not null default true,
      add column if not exists show_pod boolean not null default true,
      add column if not exists show_bol boolean not null default true,
  ...
  await client.query(`
    alter table public.portal_containers
      ...
      add column if not exists manual_extra_charge_responsibility text,
  ...
  await client.query(`
    alter table public.portal_warehouse_details
      ...
      add column if not exists manual_customer_note text,
  ...
  await client.query(
    `
      ...
      from jsonb_to_recordset($1::jsonb) as customer(
        ...
        code text,
        name text,
        inventory_remaining_pallets integer
      )
      on conflict (source_customer_id)
      do update set
        code = excluded.code,
        name = excluded.name,
        inventory_remaining_pallets = excluded.inventory_remaining_pallets,
        source_active = true,
        synced_at = now(),
        updated_at = now()
    `,
    [JSON.stringify(
      customers.map(({ id, code, name, inventoryRemainingPallets }) => ({
        source_customer_id: id,
        code,
        name,
        inventory_remaining_pallets: Number(inventoryRemainingPallets ?? 0),
      })),
    )],
  );
}
    ...
    await client.query(
      `
        ...
        do update set
          ...
          source_unload_date = excluded.source_unload_date,
...
async function recordWarehouseDetailChangeEvents(client, details) {
  for (const chunk of chunks(details, 2000)) {
    await client.query(
      `
        with incoming as (
        ...
        changed as (
          select
            ...
            current.source_order_detail_id,
            null::text as source_appointment_line_id,
            null::text as source_appointment_id,
            incoming.warehouse_point,
            'warehousePoint'::text as field_name,
            case
              when incoming.delivery_nature = '转仓' then '转仓仓点'
              else '送仓地点'
            end::text as field_label,
            current.source_warehouse_point::text as old_value,
            incoming.warehouse_point::text as new_value,
            'updated'::text as change_type
          from incoming
          join public.portal_warehouse_details current
            on current.source_order_detail_id = incoming.source_order_detail_id
          where current.source_warehouse_point is distinct from incoming.warehouse_point
          ...
          select
            ...
            'deliveryNature'::text,
            '送仓性质'::text,
            current.source_delivery_nature::text,
            incoming.delivery_nature::text,
            'updated'::text
          from incoming
          join public.portal_warehouse_details current
            on current.source_order_detail_id = incoming.source_order_detail_id
          where current.source_warehouse_point is not distinct from incoming.warehouse_point
            and current.source_delivery_nature is distinct from incoming.delivery_nature
          ...
          join public.portal_warehouse_details current
            on current.source_order_detail_id = incoming.source_order_detail_id
          where current.source_warehouse_point is not distinct from incoming.warehouse_point
            and current.source_delivery_nature is not distinct from incoming.delivery_nature
            and incoming.delivery_nature = '转仓'
            and current.source_notes is distinct from incoming.notes
        )
        insert into public.portal_source_change_events (
          source_order_id,
          source_order_detail_id,
          source_appointment_line_id,
          source_appointment_id,
...
async function recordWarehouseAppointmentChangeEvents(client, appointments) {
  for (const chunk of chunks(appointments, 2000)) {
    await client.query(
      `
        with incoming as (
        ...
        changed as (
          select
            current.source_order_id,
            current.source_order_detail_id,
            current.source_appointment_line_id,
            incoming.source_appointment_id,
            incoming.warehouse_point,
            'appointmentNumber'::text as field_name,
            'ISA号码'::text as field_label,
            current.source_appointment_number::text as old_value,
            incoming.appointment_number::text as new_value,
            'updated'::text as change_type
          from incoming
          join public.portal_warehouse_appointments current
            on current.source_order_id = incoming.source_order_id
           and current.source_order_detail_id = incoming.source_order_detail_id
           and current.source_appointment_line_id = incoming.source_appointment_line_id
          where current.source_appointment_number is distinct from incoming.appointment_number
          ...
          select
            ...
            incoming.source_appointment_id,
            incoming.warehouse_point,
            'deliveryDate'::text as field_name,
            '送仓日'::text as field_label,
            current.source_delivery_date::date::text as old_value,
            incoming.delivery_date::timestamptz::date::text as new_value,
            'updated'::text as change_type
          from incoming
          join public.portal_warehouse_appointments current
            on current.source_order_id = incoming.source_order_id
           and current.source_order_detail_id = incoming.source_order_detail_id
           and current.source_appointment_line_id = incoming.source_appointment_line_id
          where current.source_delivery_date::date is distinct from incoming.delivery_date::timestamptz::date
        )
        insert into public.portal_source_change_events (
          source_order_id,
          source_order_detail_id,
